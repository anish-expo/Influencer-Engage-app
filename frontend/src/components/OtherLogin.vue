    ...
    methods: {
      ...
      axios.post('http://127.0.0.1:5000/other_login', this.form)
        ...
        .catch(error => {
          if (error.response && error.response.data) {
            ...
            this.errorMessage = 'An error occurred during login. Maybe you are Not Approved.';
      ...
      redirectBasedOnRole(access_token) {
        ...
        if (userRole === 'Influencer') {
          ...
          this.$router.push({ name: 'InfluencerDashboard', params: { username } });
        }else if( userRole == 'Sponsor') {
            const username = localStorage.getItem('username');
          this.$router.push({ name: 'SponsorDashboard', params: { username } });
        } else {
